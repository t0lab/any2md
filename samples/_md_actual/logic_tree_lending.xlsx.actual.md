...
*(SmartArt được nhúng bên dưới — có thể click chọn và chỉnh sửa qua SmartArt Tools)*
...
## Sheet 2: Logic Customer - Quy trình xét duyệt tín dụng
...
**[Image - xlsx_sheet #2, "Logic Cus"]**
Flowchart titled "LOGIC CUSTOMER - CREDIT DECISION FLOW". Layout: vertical process flow with decision branches.

- **Start:** "START: Tiếp nhận hồ sơ vay"
- **Step 1:** "B1. Định danh KYC & hồ sơ pháp lý"
- **Decision:** "KYC hợp lệ?"
    - No -> "REJECT: KYC fail"
...
- **Decision:** "Score ≥ 600?"
    ...
    - Yes (Left branch) -> "B4. Đánh giá TSĐB (nếu có)" -> "B5. Tính LTV & Risk Rating"
- **Decision (from B3):** "DTI < 50%?"
    - No -> "REJECT: High DTI"
    - Yes -> "Tổng hợp: Approve?"
- **Decision (from B5 and DTI check):** "Tổng hợp: Approve?"
    - Yes -> "APPROVE → Giải ngân"
    - No -> "REJECT → Thông báo KH"

---

## Sheet 3: Timeline - Quy trình cho vay (End-to-End)
